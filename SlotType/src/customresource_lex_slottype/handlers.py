...
def update_handler(
    ...
    if requested_model is None or requested_model.Name is None:
        raise exceptions.InvalidRequest("Name property is a required field")
    api_interface = ApiInterface(session, resource)
    existing_model = api_interface.read_model(requested_model)
    requested_model.Checksum = existing_model.Checksum
    api_interface.update_model(requested_model)
    LOG.info("Slot Type updated")
    return read_handler(session, request, callback_context)
